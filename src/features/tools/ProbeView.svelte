...
<div class="page probe-page" bind:this={page}>
  ...
  <section class="probe-history-section" aria-labelledby="probe-history-heading">
    <div class="section-heading-row">
      ...
      <button class="button secondary compact" type="button" disabled={$probeHistory.length === 0} onclick={clearProbeHistory}>
          ...
          <li class:failed={!entry.ok}>
            <header>
              <div class="probe-result-title">
                ...
                <strong>{$t(entry.ok ? 'probe.history.success' : 'probe.history.failure')}</strong>
              </div>
            ...
            {#if entry.viaHash && entry.interfaceName && entry.interfaceType}

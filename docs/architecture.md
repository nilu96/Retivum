...
The Network visualizer is a renderer-side projection of those same snapshots and the application-owned interface, contact, and known-destination stores. It uses the same destination-presentation resolver as Path management, combining worker inventory classification with persisted destination metadata so application icons and available actions remain consistent even when a destination has no persisted application record. It shows the active local identity, configured or path-referenced interfaces, immediate next-hop hashes when available, and bounded destination-route nodes with hop counts only when more than two hops leave intermediate routers unshown. Destination context actions copy or probe the hash, request a replacement route, or drop the cached path. Immediate-next-hop actions copy the transport identity hash, probe its derived `rnstransport.probe` destination when that identity can be recalled from verified announce state, or drop every cached path using that next hop after confirmation. Probe identity lookup mirrors Reticulum's identity-hash recall behavior and keeps destination derivation in the worker. Configured-interface context actions enable or disable the interface and can remove its stored configuration after confirmation; path-referenced interfaces without a stored configuration remain read-only. The graph is rendered by the locally bundled Svelte Flow package through an app-owned adapter and circular, icon-led custom nodes. Its deterministic seed layout divides 360 degrees equally among the local identity's children; every other branch node counts its incoming parent edge alongside its children, yielding 180-degree spacing for one child, 120 degrees for two, 90 degrees for three, and so on. Child-orbit radii are sized recursively according to chord distances and sibling-circle sizes. Crowded leaf groups are packed across an automatically selected number of concentric edge-length layers. The layout evaluates coprime layer orders and radial spacing candidates, solves the exact pair-distance constraint for each candidate, and chooses the valid arrangement with the smallest outer radius; mixed branches retain the simpler two-radius fallback. Smaller groups stay on one orbit. Pair geometry is precomputed, and a small bounded in-memory cache reuses both radial and force-settled layouts while topology and labels are unchanged, so search and other presentation-only changes do not rerun the expensive layout passes. The initial automatic fit waits for the worker's complete path-management snapshot instead of the earlier aggregate runtime status, ensuring asynchronously restored routes are rendered before Svelte Flow calculates the viewport. An internal child that owns another orbit is placed beyond the parent orbit by the child radius plus an explicit gap; sibling internal circles are checked at their assigned angles and expand the parent orbit when necessary. A one-shot force pass preserves those computed edge lengths while applying repulsion, label-aware collision spacing, and weak centering; angular anchors keep the recursive structure recognizable. No runtime CDN or remote graph service is involved. Nodes can be dragged to adjust the current view, but those presentation-only positions are not persisted. Search updates only presentation data and preserves live dragged coordinates, including for nodes retained when the rendering cap changes the visible set; topology and hop-filter changes can reapply the automatic layout, while Fit network always restores it explicitly. It never imports Leviculum or platform APIs into the feature, invents missing intermediate routers, or presents the graph as a complete physical topology. Search and hop filters are local presentation operations, and a rendering cap prevents an unexpectedly large untrusted path table from monopolizing the UI thread.

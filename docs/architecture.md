...
- Transport adapters own disconnect detection and reconnection; the renderer does not classify pause/resume sessions. When an interface becomes online, Retivum preserves Leviculum's interface-targeted announces for every locally owned destination except the LXMF delivery destination. It suppresses that one implicit LXMF announce and replaces it with a metadata-complete application-controlled announce: every interface emits one targeted LXMF announce on its first online transition in a runtime, while later online transitions follow the persisted **Announce after reconnecting** option of that individual interface and remain targeted to that interface. The option defaults to enabled for new interfaces and for existing configurations that predate the option; users can override it in the interface's advanced settings. Manual and scheduled LXMF announces remain broadcasts across all online interfaces.

...
| Interfaces | WebSocket is available in browsers and Electron but hidden on iOS/Android; RNode uses native BLE on mobile/Electron, Web Bluetooth in supported browsers, or Web Serial on desktop/supported browsers; raw TCP and UDP use pinned Capacitor plugins on mobile and allowlisted `node:net`/`node:dgram` IPC bridges in Electron |
...
- Per-interface IFAC registration with a serial/radio or network media class; an omitted size uses Leviculum core's matching default, while explicit sizes must be from 1 through 64.
...
- Transport adapters own disconnect detection and reconnection; the renderer does not classify pause/resume sessions. Retivum suppresses every announce produced implicitly when Leviculum marks an interface online. It then sends only a metadata-complete LXMF delivery announce when that identity's delivery destination is new to the particular interface/network, the last successful announce has a different metadata fingerprint, or the interface's persisted **Announce after reconnecting** option is enabled. The fingerprint covers the display name, stamp cost, compression support, and announce-data format version, so metadata changed while an interface was offline is announced on its next connection. No other local destination is implicitly announced. First-announcement history is scoped to the managed identity and keyed by stable interface ID, material network fingerprint, and destination hash, so it survives reloads, application restarts, identity switches, and controlled runtime rebuilds. Manual and scheduled announces use the same metadata-complete LXMF delivery announce, broadcast across all online interfaces, and update the interfaces' delivery-destination history independently of reconnect policy.
...
| iOS/Android | Pinned `@capacitor-community/bluetooth-le` with an app-owned scan picker and direct native byte adapter | Unsupported and hidden | Pinned `@devioarts/capacitor-tcpclient` native connection | Pinned `capacitor-udp-socket` native datagrams with an app-owned adapter |
| Electron | App-owned fixed-NUS preload/main bridge backed by native CoreBluetooth, WinRT, or BlueZ bindings | Chromium Web Serial with the app-owned trusted permission/selection handler; a future native serial bridge can implement the same byte port | App-owned preload/main bridge backed by `node:net` | App-owned preload/main bridge backed by `node:dgram` |

Device selection occurs from the RNode editor under a user gesture. Browser builds retain the exact `BluetoothDevice` returned by the system selection only in session and reuse it for the first GATT connection, matching the browser reference flow. Capacitor and Electron builds show a Retivum-owned list populated by native scan events and persist the selected native device ID/name. The durable record never contains an opaque live device object. On restart, native builds silently rediscover a bonded configured device before reconnecting, while browser builds reopen only previously authorized devices.

The **Local RNode configuration** tool is capability-gated by the platform's serial and BLE support. It lists serial ports already authorized by the origin that do not report a disconnected physical/logical state, configured BLE RNodes, and browser-authorized BLE devices, associates them with matching configured interfaces, and offers the applicable system picker for a new serial or Bluetooth device. Electron device refreshes use only configured native BLE IDs and the Noble bridge; they never query Chromium's separate Bluetooth device inventory while Noble owns the peripheral. BLE uses the same fixed Nordic UART Service byte transport as ordinary RNode interfaces across Web Bluetooth, Capacitor, and Electron. A configured serial or BLE interface is exclusively claimed before maintenance: the renderer host closes its `RNodeHost`, reports the stable interface ID offline to the worker, ignores ordinary reopen commands while the claim is active, and restores the current enabled configuration when the tool disconnects or unmounts. A failed standard RNode KISS detection handshake is reported as a connection error. Live `SerialPort` and `BluetoothDevice` handles stay session-only and never enter persistence or the worker.

General Node Config is a standard RNode capability and does not depend on microReticulum provisioning. One transport-neutral maintenance session and the shared bounded KISS deframer handle device identification, unsolicited radio/channel/battery/temperature telemetry, live radio reads, persisted HOST/TNC boot mode, radio writes, Bluetooth control and pairing PINs, set-only Wi-Fi/display commands, EEPROM reads/backups/restore/wipe, and logs over serial or BLE. Its serialized write queue prevents BLE characteristic chunks from interleaving when requests are concurrent. EEPROM restores accept only an exact 200-byte reserved-region backup, require the device information lock to be clear after a wipe, write the lock byte last, and verify the complete region by reading it back. The Device tab owns identification and live status, optionally enriching them with schema version and reboot state from compatible extended provisioning; Node Config contains settings only. Connecting and disconnecting are explicit transitional states in both the Device badge and a persistent activity toast, which resolves in place to the final success or error notification. The Logs tab becomes available as soon as a maintenance connection attempt begins, independently of extended-provisioning discovery, so startup output captured during the KISS handshake is immediately inspectable. After an established serial maintenance connection is interrupted, the session inserts a visible disconnect marker into that device-log stream, keeps its configured-interface claim and log sink, and retries indefinitely with a short capped backoff; each attempt refreshes the authorized serial-port handle so USB re-enumeration after an unplug or device reset can recover. Reconnection owns one persistent activity toast whose cancel action, explicit Disconnect, and tool unmount all stop recovery and restore any claimed configured interface. After a successful maintenance disconnect, an enabled configured interface begins restoring without extending the disconnecting state; the completion toast identifies that restoration, while the ordinary interface lifecycle owns its eventual online or retry status. Tab visibility otherwise follows confirmed session capabilities: disconnected state exposes only Device, a standard RNode adds Node Config, and compatible local provisioning adds Extended provisioning. Parameterless legacy commands carry the firmware-required `0x00` sentinel. Board-specific commands remain visible but are described as conditional because standard firmware safely ignores them when the target lacks the corresponding Bluetooth, Wi-Fi, display, or NeoPixel hardware.

Running microReticulum or compatible firmware can additionally expose the existing provisioning MessagePack envelopes locally as KISS request/response commands `0x86`/`0x87`; firmware log text remains a separate `0x80` frame. This is an optional **Extended provisioning** layer, not the general RNode configuration. The local client subclasses the same schema/state client used by Remote provisioning, so field parsing, staged save/commit semantics, schema caching, and errors do not fork. The UI renders schema-provided names as text, treats them as untrusted, and confirms write-only commands. The local Logs tab buffers only plain-text `0x80` device output and offers the firmware severity thresholds plus a case-insensitive text filter; Retivum's own structured diagnostics remain in the separate Reticulum Logs tool. Standard RNode firmware that does not implement extended provisioning retains the complete general Node Config surface.

BLE bonding remains platform-owned. Android explicitly requests a bond through the pinned Capacitor BLE plugin so the system can show the RNode passkey prompt. Electron lets CoreBluetooth present the macOS system sheet, supplies the entered PIN to a custom WinRT pairing ceremony on Windows, and temporarily registers a device-scoped BlueZ `Agent1` on Linux. The agent accepts only the selected device and the fixed RNode NUS service, and the PIN is never persisted or logged. Because RNode firmware can disconnect after accepting a new bond, authorization retries connection and Nordic UART service discovery with bounded delays before reporting failure. When a mobile application returns to the foreground, every active native BLE RNode is checked against the operating system's connected-device inventory; a stale link enters the normal reconnect path immediately, while an interface already awaiting reconnection retries without waiting for a suspended timer. Every Electron GATT open rediscovers the durable device ID before using its NUS characteristics, clears cached native peripherals when the adapter leaves the powered-on state or a device disconnects, removes listeners belonging to the completed connection generation, retries failed opens on every desktop platform, and bounds native disconnect cleanup. This prevents a stale Noble wrapper or late disconnect event from surviving into a replacement connection.
...
The minimal shell in `electron/main.mjs` loads only the packaged `dist/index.html`, blocks renderer navigation, keeps Node integration disabled, and installs the device-access, BLE, and socket handlers. Electron BLE, TCP, and UDP use dedicated main-process modules plus `electron/preload.cjs` with context isolation. Main-process registration requires an exact trusted-renderer predicate, validates IDs/endpoints/byte arrays, associates every connection with its owning `webContents`, and exposes only transport-specific scan/pair/open/send-or-write/close/event methods. The BLE bridge fixes the service and characteristic UUIDs to RNode NUS instead of exposing generic Bluetooth access. Ordinary web builds have no native bridge.
...
- Conversation order is locally segmented rather than globally sorted by the sender-reported LXMF timestamp. Each outgoing message starts an immutable boundary; incoming messages received afterward remain in that segment and are sorted by sender timestamp with a persistent local receive sequence as the tie-breaker. A delayed message can therefore move within the currently open incoming block but can never cross a local reply. Segment and receive-sequence assignments are persisted once at ingestion. Additional boundary rules belong to the pure domain ordering policy and affect only newly ingested messages.
...
interface LxmfPreferences {
  ...
  propagationSyncOnResume: boolean; // false on first run
...
- For direct or opportunistic delivery, propagation is an independently configurable fallback that is enabled by default. Once the primary attempt reaches terminal `failed`, Retivum queues the same logical message as propagated. Selecting **propagated** skips the primary attempt and presents fallback as unavailable without changing its saved preference.
...
| Tools | Primary navigation directory for the local Network visualizer, Reticulum logs, Path management, Remote provisioning, live Status details, and raw destination Probing |
...
Path management projects the worker-owned path table and verified known-destination state into two read-only snapshots. Users can broadcast a path request for a manually entered or listed destination, remove one or all cached paths, and forget one or all known destinations. When known destinations are grouped by their shared public identity, the group heading displays the projected 16-byte identity hash rather than an abbreviated public key. Replacement path requests first drop the cached route and use one shared per-destination operation and cooldown across Path management and the Network visualizer. A path request remains a cancellable live activity for the shared 20-second discovery window: a matching verified `pathFound` event completes it successfully, expiry completes it with an error instead of treating packet submission as discovery success, and cancellation removes the pending worker job so a later path event cannot complete the cancelled activity. Forgetting a destination atomically removes its remembered public identity, verified raw announce, known ratchet, propagation metadata, and matching path from the encrypted global network checkpoint. It does not delete application-owned chats, contacts, messages, or bookmarks; a later verified announce or explicit user action can learn the destination again.

The Destination hash generator accepts a 16-byte identity hash and an editable, case-sensitive full aspect name, then follows Reticulum's reference derivation: SHA-256 of the UTF-8 name truncated to 10 bytes, concatenated with the identity hash, followed by SHA-256 truncated to 16 bytes. Its combobox suggests only full aspect names already classified by Retivum while allowing custom names. Results are kept newest-first in a bounded, in-memory session history below the generator; regenerating the same normalized identity/name combination moves it to the front instead of adding a duplicate. Each result exposes a visible destination-hash copy action. Its context menu can copy the source identity hash or aspect name and start the same shared path-request activity used elsewhere. Direct requests observe the runtime status plus the global per-destination pending and cooldown state used by Path management and the Network visualizer.

The Network visualizer is a renderer-side projection of those same snapshots and the application-owned interface, contact, and known-destination stores. It uses worker inventory classification together with exact persisted destination metadata so application icons and available actions remain consistent even when a destination has no persisted application record. Identity-shared display names are deliberately excluded from this projection: remote identities are labeled by their abbreviated identity hash, the local identity uses its configured local name, and destinations use an exact local contact name, an exact destination announcement name, or their destination hash. It shows the active local identity, configured or path-referenced interfaces, immediate next-hop hashes when available, and bounded destination-route nodes with known hop counts from two hops onward. Optional identity grouping uses the verified public key from the worker inventory, matching Path management's identity grouping. An ordinary identity-group node appears only when at least two visible paths belong to destinations for the same public key; singleton identities remain destination nodes, while known transport identities can still replace transport occurrences. Ordinary identity destination children are collapsed by default; the identity badge shows the total unique visible destination count for the public key and activating the identity toggles its destination orbit. Identity expansion is deliberately local: every existing node remains pinned at its current on-screen position and only newly revealed destinations are translated around the selected identity. While a group remains expanded, its identity stays pinned. Adding a highlighted destination recalculates the group's orbit and redistributes automatically positioned children around that fixed identity, while manually dragged children keep their positions; an unhighlighted addition and path removal leave retained children in place. An expanded group also applies the same visual path highlighting as an exact identity-hash search without changing the search field or its result count; an explicit text search takes visual precedence. Destination entry and exit are animated, while reduced-motion preferences disable the transition. Fit network collapses every identity group, restores the complete collision-safe recursive arrangement for that compact topology, and fits it into the viewport. The worker also projects the Reticulum identity hash derived from each verified public key; when that identity hash is an immediate next hop, the renderer replaces that next-hop occurrence with one shared identity node instead of adding another node at the same ingress. Ordinary identity nodes use the same person symbol and blue color as the local identity. An occurrence that replaces an immediate transport node, or places that known transport identity directly below another interface, retains the orange transport color, route symbol, and transport-node actions, remains non-expandable without a count badge, and always displays its children. Other occurrences of that same public identity remain ordinary blue, expandable identity nodes. Such a transport identity can own its local destinations and connect onward to other grouped identities. Consolidated incoming edges retain the shortest known hop count. Destination context actions copy or probe the hash, request a replacement route, or drop the cached path. Immediate-next-hop actions copy the transport identity hash, probe its derived `rnstransport.probe` destination when that identity can be recalled from verified announce state, or drop every cached path using that next hop after confirmation. Probe identity lookup mirrors Reticulum's identity-hash recall behavior and keeps destination derivation in the worker. Configured-interface context actions enable or disable the interface and can remove its stored configuration after confirmation; path-referenced interfaces without a stored configuration remain read-only. The graph is rendered by the locally bundled Svelte Flow package through an app-owned adapter and circular, icon-led custom nodes. Its deterministic seed layout divides 360 degrees equally among the local identity's children; every other branch node counts its incoming parent edge alongside its children, yielding 180-degree spacing for one child, 120 degrees for two, 90 degrees for three, and so on. Child-orbit radii are sized recursively according to chord distances and sibling-circle sizes. Identity grouping rebuilds the rooted topology and runs that same recursive sizing pass again after grouped identity branches have replaced destination leaves; expanded identity destination orbits are therefore propagated through every ancestor before positions are assigned. The grouped graph retains this exact collision-safe radial geometry instead of applying the generic force pass afterward, which could otherwise pull reserved child circles through one another. Any crowded child group, including recursively sized identity subtrees, is packed across an automatically selected number of concentric edge-length layers. The layout evaluates coprime layer orders and radial spacing candidates against each child's individual circle radius, solves the exact pair-distance constraint for each candidate, and chooses the valid arrangement with the smallest outer radius. Smaller groups stay on one orbit, while mixed non-crowded branches retain the simpler two-radius fallback. Pair geometry is precomputed, and a small bounded in-memory cache reuses both radial and force-settled layouts while topology and labels are unchanged, so search and other presentation-only changes do not rerun the expensive layout passes. The initial automatic fit waits for the worker's complete path-management snapshot instead of the earlier aggregate runtime status, ensuring asynchronously restored paths are rendered before Svelte Flow calculates the viewport. An internal child that owns another orbit is placed beyond the parent orbit by the child radius plus an explicit gap; sibling internal circles are checked at their assigned angles and expand the parent orbit when necessary. For the ungrouped graph, a one-shot force pass preserves those computed edge lengths while applying repulsion, label-aware collision spacing, and weak centering; angular anchors keep the recursive structure recognizable. No runtime CDN or remote graph service is involved. Nodes can be dragged to adjust the current view, but those presentation-only positions are not persisted. Search updates only presentation data and preserves live dragged coordinates, including for nodes retained when the rendering cap changes the visible set; topology, identity grouping, and hop-filter changes can reapply the automatic layout, while local identity expansion preserves the current arrangement and Fit network always restores the collapsed automatic layout explicitly. It never imports Leviculum or platform APIs into the feature, invents missing intermediate routers, or presents the graph as a complete physical topology. Search and hop filters are local presentation operations, and a rendering cap prevents an unexpectedly large untrusted path table from monopolizing the UI thread.

Identity grouping treats one public key as one logical expansion state but may project it into several visual occurrences when its destinations use different ingress interfaces or immediate transport nodes. An ordinary expandable identity occurrence exists only when the public key owns at least two visible destinations; a singleton remains its original destination node without an identity wrapper or count badge. Every expandable identity occurrence displays the total unique visible destination count for that public key, while expanded child destinations are partitioned by their active ingress path and therefore appear exactly once. Activating any expandable identity occurrence expands, highlights, or collapses every ordinary occurrence for the public key; transport occurrences never toggle, remain badge-free, and keep their children visible. Animations originate only from each child's ordinary expandable occurrence, and Escape, outside click, or Fit network collapse the logical identity globally without moving or hiding transport children. Transport presentation and actions apply both to occurrences that replace actual immediate next-hop nodes and to occurrences of that known transport identity which are direct children of an interface. An occurrence reached through a different transport remains an ordinary blue identity when the identity has multiple destinations. This presentation does not turn a solid identity-membership edge into an additional network hop. Route-specific occurrence IDs include the public key and ingress branch so manual positions remain stable while that path remains present. Solid identity-membership edges never display hop badges. Ordinary paths through a transport node remain dashed—including two-hop paths—and display their known hop count.

Local destinations are projected from the WASM runtime's IN-destination hash list. The worker identifies the active LXMF delivery destination by its separately returned delivery hash; other local hashes remain application-unknown unless another trusted application-owned source can classify them.

Status details keeps maximum bitrate separate from current traffic. For RNode interfaces, Max bitrate is the deterministic theoretical LoRa PHY rate calculated from spreading factor, coding rate, and bandwidth. For TCP, UDP, and WebSocket interfaces, it is the highest Current RX or Current TX value observed during the current interface runtime. Current RX and Current TX use separate five-second wall-clock windows and correctly decay to zero when traffic stops, while Max bitrate retains the highest observation. Incoming and outgoing announce counts are cumulative for the current interface runtime and are displayed separately before their sampled per-second rates. On mobile, each interface card initially shows its first four summary metrics; a per-card disclosure reveals the remaining summary and radio metrics.
...
| `interfaces` | Stable UUID, immutable creation timestamp, discriminated type configuration, persisted enabled state, and plaintext IFAC network name, passphrase, and optional size override; shared across managed identities, always presented in creation order, and empty on first run |
...
| `messages` | `identityId`, local ID, optional LXMF ID, normalized content, direction, state, timestamps, selected delivery method, and persistent local segment/receive-sequence ordering |
...
Release tests cover stable vault access, bridge availability outside the worker, application-origin stability, reinstall/upgrade behavior, key rotation, and recovery when the vault entry is lost. Private identity material and IFAC passphrases must never appear in renderer IPC errors, logs, crash reports, URLs, clipboard operations without explicit action, or support bundles.

For v1, messages, contacts, heard-announce observations, settings and interface configurations (including IFAC passphrases), Nomad cache, and redacted diagnostics are deliberately stored unencrypted in the application sandbox/database. Managed private keys and identity runtime snapshots are encrypted. A user-requested interoperable identity export is the standard raw 64-byte Reticulum private identity file and is necessarily unencrypted; it requires an explicit warning and must never be created automatically. A future full application/state backup must use an approved protected envelope. This limitation must be stated in the localized privacy/security UI. Full database encryption is deferred and is not an implied property of SQLite or IndexedDB.
